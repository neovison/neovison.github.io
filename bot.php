<?php
    ...
    if ($temp_link[0] != $album) {
        file_put_contents($conf['temp_link'], $alnum);
        require "photos.php";
        return true;
    }
        ...
        case "message_new":
            if($mess == $conf['mess']){
                ...
                curl_setopt_array($myCurl, array(
                    CURLOPT_URL => $conf['apiurl'].'messages.send?user_id='.$u_id.'&group_id='.$conf['group_id'].'&attachment='.$photos_all[mt_rand(0,count($photos_all))].'&message='.urlencode('Твой пукан, ').urlencode($user_name).'&v='.$conf['v'].'&access_token='.$conf['standalone'],
